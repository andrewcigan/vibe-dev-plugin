...
case "$TOOL" in
  Write|Edit|MultiEdit)
    ...
    # folder-scope (v7 P9 → v8 L5-F3): запись ВНЕ корня -> WARN по умолчанию (промоушн из log-only).
    # Все профили, безвредно (warn, не block; block отложен до замера % ложных на корпусе).
