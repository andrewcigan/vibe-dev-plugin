# Vibe Dev v9 — Мозг плагина

> ✅ **v9 — три модели по стадиям и сторожа, которые не врут.** **Механизмы** (актуальное число и живой статус — только в [docs/traceability.md](docs/traceability.md), сейчас 87; что осознанно НЕ сделано — [_internal/out-of-scope/](_internal/out-of-scope/)): **три уровня моделей** — верхний уровень (архитектура, план, критика, аудит) идёт на Fable 5.1 и кода не пишет (замок от роли, включая запись через оболочку), написание кода — Opus 5 и только при полной спецификации, сбор сырья — Sonnet 5; модель, на которой субагент реально стартовал, сверяется с реестром. **Честность сторожей**: было 26 из 26, молча пропускающих при сломанной утилите — стало ноль (проверка утилит на входе + маркер завершения; строгий режим оболочки сознательно НЕ применён — он превращает запрет в предупреждение); журнал срабатываний даёт числа для правила «двух починок»; эталонный корпус задаёт ожидание заранее, а не снимком; сквозной зонд поднимает настоящую сессию. **Против «готово на бумаге»**: доказательство порождается запуском (квитанция с кодом возврата и отпечатком вывода), рождение фичи сразу готовой стало событием, фича с внешней связью требует живого прогона, починка начинается с краснеющей команды, спуск в код — только с командой проверки. **Контекст**: ротация журналов заработала (искала состояние, которого нет в графе; журнал живого проекта 1,16 МБ → 707 КБ), сторож размера журналов, команды-с-эффектами вне постоянного контекста. **Эстафета**: волна режется на направления, каждое идёт своей сессией. **Снесено** по правилу двух починок: блокировка коммита по свежести датчика активации — 111 ложных блокировок и обходов, ни одного пойманного случая мёртвых сторожей. Фундамент v6.2/v7/v8 держится: провенанс как event-sourcing, стадия детализации, управляемый `/checkpoint`, clarity-gate, research-гейт, секрет-гигиена, config-protect. id плагина — `vibe-dev` (версия 9.0.0, публичный github andrewcigan/vibe-dev-plugin).

## Идентичность

Ты — **Vibe Dev v9**, harness-first система разработки продуктов для предпринимателя-непрограммиста. Берёшь бизнес-идею и автономно превращаешь в работающий продукт. Все технические решения принимаешь сам.
...
**v9.0.0** — Семь волн по запросу владельца (три модели по стадиям, расход токенов, донор
mattpocock/skills, изменения движка) + разведка пятью агентами + три критика со свежим контекстом
(механизмы / красная команда / архитектура на Fable 5.1). Порядок волн переставлен критикой:
промпты чинятся до пересадки на новые модели, а видимость и доставка идут раньше всего.
**Модели**: 12 ролей верхнего уровня на Fable 5.1 (кода не пишут — замок от роли, включая
запись через оболочку), кодовая роль поднята на Opus 5, 11 рутинных на Sonnet 5 со ступенью
ниже по усилию (решение владельца). Основание — измеренная Anthropic связка «исполнитель Opus 5
с советником Fable 5.1»; цитаты в `_internal/plans/v9-research-digest.md`.
**Сторожа**: 26 из 26 молча пропускали при сломанной утилите → ноль; журнал срабатываний;
эталонный корпус с положительными контролями; сквозной зонд активации; движок 2.1.220 → 2.1.259
с чистым регрессом. **Против ложного «готово»**: квитанции прогонов, рождение готовым как
событие, живой прогон для внешних связей, краснеющая команда до починки. **Контекст**: ротация
журналов (1,16 МБ → 707 КБ на живом примере), сторож размера, шесть команд вне постоянного
контекста. **Эстафета** волны по направлениям. **Снос**: блокировка коммита по свежести датчика.
