...
LLM на вопрос «уверен?» всегда отвечает «да» — поэтому подтверждения словами не принимаются.
Каждый вердикт APPROVE по спорному месту обязан опираться на ФАКТЫ, добытые чтением:
- какие модули/функции читают и пишут эту сущность (перечислить, не «наверное используется»);
- какие реальные значения поле принимает в существующих данных (если данные есть);
- цитата требования из domain-rules/PRODUCT, которой решение соответствует.
Нет фактов — вердикт не APPROVE, а «НЕ ПРОВЕРЕНО: нужен факт X». Сам акт добычи фактов
вскрывает пропущенные сущности чаще, чем абстрактное «подумай ещё раз».
