...
# --- Починка начинается с краснеющей команды (v9 F7.1). ---
# Правило владельца: заявляй ровно ту величину, которую померил, и проверяй глазами, а не по
# косвенным признакам. Здесь оно становится условием: пока нет команды, которая ВОСПРОИЗВОДИТ
# симптом, любая гипотеза о причине — догадка, и починка идёт вслепую. Поле repro_command
# обязательно для перехода в состояние починки или отката.
REPAIR_STATES = {"fixing", "rollback"}
for feat_id, feat_state, f in all_features:
    if feat_state not in REPAIR_STATES:
        continue
    if str(f.get("repro_command") or "").strip():
        continue
    errors_soft.append(
        "%s: переход в «%s» без команды воспроизведения. Пока нет команды, которая краснеет "
        "именно на симптоме, причина — догадка, а починка идёт вслепую. Впиши repro_command: "
        "она обязана зеленеть на исправном образце и краснеть на этом. (v9 F7.1)"
        % (feat_id, feat_state))
