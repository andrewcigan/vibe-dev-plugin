...
EOF
  ...
  case "$PROFILE" in
    pending-*)
      ...
      ;;
    standard|strict)
      # --- СНЕСЕНО в v9 F1.1: блокировка коммита по свежести датчика активации. ---
      # Правило владельца «двух починок»: механизм, чинившийся дважды, третий раз не чинят,
      # пока письменно не сказано, сколько раз он предотвратил беду. Замер по журналам живых
      # проектов: 395 упоминаний датчика, 111 из них — ложные блокировки и обходы; случаев,
      # когда он поймал реально мёртвые сторожа, — НИ ОДНОГО. Плюс собственный дефект: в
      # рабочей копии git читал закоммиченную протухшую метку и останавливал коммит на
      # исправной системе.
      #
      # Цена была не нейтральной: каждый обход снимает разом ВСЕ проверки этого хука
      # (границы правок, провенанс, приёмка) — сторож научил обходить защиту целиком.
      # В журналах проектов 176 упоминаний такого обхода.
      #
      # Что осталось вместо него: блокировка при неподтверждённом профиле (ветка выше — она
      # про состояние bootstrap, а не про свежесть метки); сквозной зонд активации
      # tests/hooks/e2e-probe.sh (поднимает настоящую сессию и ищет след в журнале
      # срабатываний); сам журнал срабатываний, по которому видно, работали сторожа или нет.
      # Датчик остался как ДИАГНОСТИКА (/doctor), но коммит больше не останавливает.
      :
      ;;
    *)
