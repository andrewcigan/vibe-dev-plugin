# v9.0.0 — Три модели по стадиям и сторожа, которые не врут (2026-09-03)

Запрос владельца: посадить архитектуру и аудит на Fable 5.1 (кода не пишет), код писать на
Opus 5 при полной спецификации, резать волны на сессии по направлениям, разобраться с расходом
токенов, забрать полезное из mattpocock/skills и из изменений движка.

Разведка пятью агентами (гайды Anthropic по Fable 5.1 и Opus 5, контекст-инженерия, донорский
набор навыков, changelog движка, наши накопленные жалобы) и три критика со свежим контекстом —
по механизмам, красная команда и архитектурный на самой Fable 5.1. Критика переставила порядок:
промпты чинятся ДО пересадки на новые модели, а видимость и доставка идут раньше всего.

## Волна 0 — видимость и доставка

- **Журнал срабатываний сторожей** (`hooks/lib/hook-io.sh`, свод `scripts/guard-stats.sh`).
  До него ни для одного из 71 механизма нельзя было ответить на вопрос правила «двух починок»:
  сколько раз он предотвратил беду. Пишется из единственной точки запуска проверок.
- **Эталонный корпус** (`tests/hooks/baseline-probe.sh`): ожидание задано заранее и жёстко, а не
  снимком текущего поведения — иначе болезнь стала бы эталоном. Карта аргументов (у сторожей три
  разных протокола вызова) и положительные контроли.
- **Сквозной зонд активации** (`tests/hooks/e2e-probe.sh`): поднимает настоящую сессию и ищет
  след в журнале. Корпус проверяет скрипты и не отличит «сторожа исправны» от «движок их не зовёт».
- **Движок 2.1.220 → 2.1.259**, регресс чистый: поведение всех сторожей идентично до и после.
- Рабочий слой `.harness` игнорируется классом, а не поимённым списком из 15 строк.

## Волна 1 — снос и честный ремонт

- **Сторож больше не отвечает «возражений нет», когда проверка не выполнилась.** Замерено: 26 из
  26 сторожей при подменённых `jq`/`python3` возвращали ноль; защита от боевого ключа в исходниках
  при сломанном `jq` молчала, и ключ проходил. Стало: молчунов ноль. Механика — проверка утилит на
  входе плюс маркер завершения. Строгий режим оболочки сознательно не применён: он ломает трёх
  сторожей и превращает запрет в предупреждение.
- **Журнал фич не правится в обход record-change**: распознавание записи в команде плюс сверка
  состояния файла до и после — второй рубеж не зависит от формы команды.
- **СНЕСЕНА блокировка коммита по свежести датчика активации.** 395 упоминаний в журналах живых
  проектов, 111 ложных блокировок и обходов, ни одного пойманного случая мёртвых сторожей;
  каждый обход снимал разом все проверки хука (176 упоминаний обхода). Осталась блокировка
  неподтверждённого профиля, зонд активации и журнал.

## Волна 3 — три модели по стадиям

- 12 ролей верхнего уровня на **Fable 5.1**, кодовая роль на **Opus 5**, 11 рутинных на
  **Sonnet 5** со ступенью ниже по усилию (решение владельца).
- **Роль верхнего уровня физически не пишет код** — включая запись через оболочку: прежняя
  формулировка «только читающая роль» была неполной, шесть таких ролей имели доступ к оболочке.
- **Пин модели сверяется по факту старта субагента**; если движок не сообщает модель, сторож
  пишет об этом в журнал, и пин честно остаётся дисциплиной.
- **Спуск в код только с командой проверки**; падение разбора состояния перестало выглядеть
  чистой проверкой.

## Волна 4 — против «готово на бумаге»

- **Квитанция прогона** (`scripts/verify-receipt.sh`): код возврата, отпечаток вывода, состояние
  дерева. Прозой не напишешь. Требуется в момент перехода в готовое состояние.
- **Рождение записи сразу готовой стало событием** — так прошёл прошлый инцидент с дублями.
- **Фича с внешней связью требует живого прогона**: подменённая в тестах граница — ровно та,
  которую надо было доказать.
- **Починка начинается с краснеющей команды** (`repro_command`).

## Волна 5 — контекст

- **Ротация журналов заработала**: она отбирала записи по состоянию «done», которого в графе
  переходов нет вовсе. Механизм существовал с v8 и не срабатывал ни разу. На копии живого журнала
  1,16 МБ → 707 КБ. Сторож размера журналов называет счёт в токенах при старте сессии.
- Шесть команд, которые запускает только человек, убраны из постоянного контекста.

## Волна 6 — эстафета

- Волна режется на направления по слоям (задачи с общим файлом остаются вместе), очередь лежит в
  файле, закрытие направления открывает следующее.

## Честность

- Реестр осознанных отказов `_internal/out-of-scope/`: сокращение глобальной шпаргалки, единый
  протокол сторожей, профиль от целевого проекта, разделение осей ревью — с причинами.
- Сторож согласованности витрины: версия сверяется в манифесте, файле маркетплейса и описаниях.
- Механизмов 87, самопроверка 52 раздела, 52 теста.
