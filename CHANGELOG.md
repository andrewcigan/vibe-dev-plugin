...
## v8.0.2 (2026-07-10) — Четыре фикса по первому dogfooding (внедрение на живой проект)

Первый боевой прогон: харнес поставлен ПОВЕРХ живого продового проекта (Chrome-расширение). Агент
собрал технический отчёт с находками про поведение движка — по ним 4 точечных фикса. Цикл узаконен:
один проект → отчёт → улучшение плагина.

- **#1 — инструкция скилла `/upgrade-project` переписана.** Она описывала движок v6 (только строгий
  режим) и не знала про `--soft`/`--dry-run` — код скрипта ушёл в 8.0.1, документация отстала. Теперь
  соответствует: мягкий режим, `--dry-run` первым шагом, ссылка на `/patch-projects`.
- **#2 — рантайм-файлы харнеса больше не «грязнят» git.** Сторожа пишут служебные счётчики/логи в
  `.harness/` (`hooks-heartbeat`, `bash-repeat-state`, `feature-budget-state`, `*.log` и др.), а они
  не были в `.gitignore` → каждый проект получал вечно грязное дерево, о которое спотыкается сам
  `upgrade-project` (он требует чистое дерево). Теперь `upgrade-project.sh` дописывает секцию игнора
  в `.gitignore` проекта (идемпотентно) перед проверкой, а свою же правку из проверки исключает.
  Конфиг проекта (engine-version/profile/hook-mode) по-прежнему отслеживается git.
- **#3 — сторож смены модели больше не шумит на документации.** Он срабатывал на любой файл, где
  просто *упоминаются* модели/версии (отчёты, CHANGELOG, README, SESSION) — 100% ложные срабатывания
  на прозу. Сужен по типу файла: смена контракта вызова живёт в коде/конфиге, не в тексте.
- **#4 — граф переходов задач честно помечен справочным.** Плагин загружал `allowed_transitions`
  из схемы, но в коде их не применял (проверялось только валидное ИМЯ статуса + согласованность
  bucket↔state). Формулировка в traceability исправлена на точную; в коде — комментарий, почему граф
  намеренно не enforced (легитимные нестандартные переходы при ретрофите на живой проект не должны
  падать). Честность деклараций: заявлено = сделано.

Проверено: `test-dogfooding-fixes.sh` 10/10 (+ self-check раздел 45), self-check 45 разделов зелёные,
`plugin validate --strict`. Независимая критика fresh-context.

## v8.0.1 (2026-07-10) — Фикс ядра (C3) + безопасный перевод живых проектов на v8

Патч по итогам независимой критики (fresh-context ревьюер) перед переводом 9 живых проектов.

**Фикс ядра C3 (латентный дефект v8.0.0).** Git pre-commit гейт «правка требования требует
событие истории» (L3-F4) ошибочно считал поле `state` изменением требования. Обычный жизненный
цикл задачи (`/verify` active→passing, `/ship` passing→done, `/feature` captured→active) пишет
`state` напрямую, не через record-change → первый же коммит после `/verify` на v8-проекте
**отвергался**. Итерация `all_features` при каждой записи делала это блокирующим для всего проекта.
Фикс (`templates/git-pre-commit.sh` блок 5): событие истории требуется на правку ТРЕБОВАНИЯ
(name/description/size/invariant) и на ТЕРМИНАЛЬНУЮ судьбу (rejected/superseded/reopened); обычный
lifecycle-прогресс `state` требованием не считается. Тест `test-provenance-edit-gate.sh` дополнен
кейсами жизненного цикла (7/7), которых не было — из-за чего дефект и проскочил в 8.0.0.

**Механизм перевода живых проектов на v8 (мягкое включение).** Прямой перевод в строгий режим
ломает работу: движок проверяет всё состояние при каждой записи, и старые «готово» без формальных
доказательств блокируют первую правку. Решение — двухфазно:
- `scripts/upgrade-project.sh` переписан: флаг `--soft` (движок 8 + `hook-mode=learn` при
  СОХРАНЁННОМ `profile=strict` — v8-гейты становятся подсказками, не блокировками; hard UI-контроль
  и активность state-machine сохраняются), встроенная миграция провенанса, честный человекочитаемый
  `--dry-run` (прогон реального сторожа: что заблокируется / станет подсказкой). Бэкап-тег + отказ на
  грязном дереве перед записью; уже-строгий проект `--soft` не разжалует.
- `scripts/patch-projects.sh` — оркестратор: скан проектов (исключая плагин, бэкапы, шаблоны, тесты,
  git-worktree; кириллица/пробелы в путях), реальный предпросмотр по каждому, применение мягкого
  включения к безопасным, флаг UI-долга для проектов, где мягкий режим не помогает. Идемпотентно.
- Скилл `/patch-projects` — запуск из чата: предпросмотр → подтверждение → применение.
- `migrate-provenance.sh` помечает реконструированные этикетки `source_ref.ref=retro-migration`
  (отличимо от живого вывода). `enforcement-config-protect.sh` явно санкционирует setter-скрипты
  как легитимный путь к мягкому режиму (вместо тихого обхода).

Проверено живьём: фикс C3 — 6/6 сценариев; мягкое включение end-to-end — 9/9 (строгий deny →
мягкий warn, провенанс мигрирован, профиль сохранён, бэкап создан). self-check зелёный.
