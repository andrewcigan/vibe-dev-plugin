...
WARN	${_msg}"
  ...
  hook_log_verdict "$_cwd" "$_label" "$_rc" "$_out"
  printf '%s' "$_out"
}

# --- Журнал срабатываний (v9 F0.1). ---
# ЗАЧЕМ. До этого ни для одного механизма нельзя было ответить на вопрос правила «двух починок»:
# сколько раз он предотвратил беду, ради которой построен, и сколько раз помешал нормальной
# работе. Оба счётчика теперь пишутся здесь — в единственной точке, через которую проходит
# КАЖДЫЙ сторож. Мимо неё сторож не запускается, поэтому счёт не зависит от дисциплины.
#
# Строка на вердикт: время, сторож, вердикт (BLOCK/WARN/pass/CRASH), код возврата, событие.
# Никогда не влияет на stdout проверки и на её код возврата: всё в подоболочке, ошибки гасятся.
hook_log_verdict() {
  local _cwd="$1" _label="$2" _rc="$3" _out="$4"
  [ -n "$_cwd" ] && [ -d "$_cwd" ] || return 0
  (
    local _v="pass"
    if [ "$_rc" -ne 0 ]; then _v="CRASH"
    elif printf '%s' "$_out" | grep -q "^BLOCK"; then _v="BLOCK"
    elif printf '%s' "$_out" | grep -q "^WARN"; then _v="WARN"
    fi
    # Первые слова сообщения — чтобы в своде было видно, ЧТО именно охранялось.
    # Обрезка по СЛОВАМ, а не по байтам: cut -c рубит многобайтовый символ пополам и портит журнал.
    local _gist; _gist="$(printf '%s' "$_out" | awk 'NR==1{n=(NF>12?12:NF); s=""; for(i=1;i<=n;i++) s=s (i>1?" ":"") $i; print s}' | tr -d '"\\')"
