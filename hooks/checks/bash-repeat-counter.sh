...
# Circuit breaker (v7 Волна 4): на удвоенном пороге — твёрдая эскалация в /stuck (stuck из
# дисциплины в механизм). Паттерн из loop-engineering: цикл не сходится → размыкаем, не долбим.
CIRCUIT="${VIBE_BASH_CIRCUIT:-$((THRESH * 2))}"
TAB="$(printf '\t')"
...
# Инжект РОВНО на пороге (мягкий) и РОВНО на удвоенном (circuit breaker) — не спамить между.
if [ "$count" -eq "$THRESH" ]; then
  ...
  printf 'WARN%s🛑 Circuit breaker: команда идёт %s-й раз подряд без успеха — цикл НЕ сходится, ещё один retry ничего не изменит. Разомкни: (1) запусти /stuck (сформулирует тупик, разошлёт LLM-кворум Claude+Gemini+Codex, даст 3 подхода A/B/C) ИЛИ параллельного субагента на структурное решение; (2) если сам не двигаешься — эскалируй пользователю с 3 вариантами. Повтор той же команды дальше — это залипание, не работа.\n' "$TAB" "$count"
fi
exit 0
